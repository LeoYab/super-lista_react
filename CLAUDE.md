...
Realtime Database also has a top-level `Categories/{id}` node (shared across all users, not per-uid) read via `subscribeToCategories` and written via `addCategory`. Categories are only ever created dynamically through `resolveProductCategory` in `src/utils/categoryMapping.js` (shared by the barcode-scan handlers in `App.js` and `Supermercados.js`, replacing what used to be two duplicated copies of this logic): a scanned product's raw VTEX category path is mapped to one of a *fixed* curated set of main categories per brand (`CARREFOUR_CATEGORIES`/`CHANGOMAS_CATEGORIES`, sourced from each retailer's own mega-menu — see git history for how they were extracted) via `mapToCarrefourCategory`/`mapToChangoMasCategory`, then found-or-created by title in the flat `Categories` node — so scanning never spawns a new one-off category from a subcategory name, it always lands in a main category. Which brand's mapper/curated set is active is driven by GPS-detected supermarket (`App.js`) or the explicitly selected brand tab (`Supermercados.js`); `ProductForm`'s manual category picker on the main list screen additionally merges in the GPS-detected brand's curated categories (`getCategorySetForBrand`) alongside the saved Firebase list, via `formCategories` in `App.js`.
