...
Realtime Database also has a top-level `Categories/{id}` node (shared across all users, not per-uid) read via `subscribeToCategories` and written via `addCategory` — new categories can be created dynamically (e.g. when a scraped product's category doesn't map to an existing one; see `mapCarrefourCategory` in `Supermercados.js`).
