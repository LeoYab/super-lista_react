...
import ScannerView from '../BarcodeScanner/ScannerView';
...
const Supermercados = () => {
    ...
    const searchProduct = async () => {
      ...
      if (isLiveSupported) {
        try {
          ...
          if (apiProduct) {
            const resolvedCat = await resolveProductCategory(apiProduct.categories, categories, brandId, addCategory);
  ...
  const scannerRef = useBarcodeScanner(showScanner, onScanSuccess, (err) => {
  ...
  return (
    <div className="supermercados-container">
      <div className="supermercados-header" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
        <Button
          onClick={() => navigate('/')}
          variant="secondary"
